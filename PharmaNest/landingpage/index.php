
<!DOCTYPE html>
<html>
<head>
	<meta charset="utf-8">
	<meta name="viewport" content="width=device-width, initial-scale=1">
	<title>PHARMANEST ESSENTIAL</title>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.1/css/all.min.css">
	<link rel="stylesheet" href="landing.css">
</head>
<body>
	<nav class="navbar">
		<div class="navdiv">
        <div class="logo">
            <img style="width: 100px; float: left; border-radius: 100%; border: solid 1px rgb(0, 0, 0); margin: 1.5rem; margin-left: 3rem;" 
            src="logo.png" alt="PHARMANEST ESSENTIAL LOGO">
        </div>
        <div class="title">
        <h1> PHARMANEST ESSENTIAL </h1>
        </div>
            <div class="menu">
                <ul>
                    <li><a href="#">Home</a></li>
                    <li><a href="#">About</a></li>
                    <li><a href="#">Contact</a></li>
                    <li><a href="#">SignUp</a></li>
                </ul>
            </div>
		</div>
	</nav>
<div class="photo">
    <div class="descript">
        <h3>
        Welcome to PharmaNest Essentials
        Your trusted online pharmacy, delivering quality and healthcare products.
        At PharmaNest Essentials, we prioritize your health and well-being. 
        With a wide range of prescription medicines, over-the-counter products, 
        and wellness supplements, we make it easy and convenient for you to manage 
        your healthcare needs.
        </h3>
    </div>
</div>

<div class="container">
    <h4>
    <h3>What We Offer</h3>
    <h3>Prescription Medications</h3>
        Access affordable, FDA-approved medications with ease.
<br>
    <h3>Vitamins & Supplements</h3>
    Boost your health with our carefully curated selection of vitamins, minerals, and dietary supplements.
<br>
    <h3>Personal Care & Wellness</h3>
    Explore skincare, haircare, and other essential products to support your daily health and beauty routines.
    </h4>
</div>
<center>
<div class="containers">
<h4>
    <h3>Stay Connected</h3>
Subscribe to our newsletter for exclusive deals, healthcare tips, and updates.
<br>
Start your journey to better health today.
    </h4>
<br>
    <a href="#" class="fas fa-facebook" aria-hidden="true"></a>
    <a href="#" class="fas fa-mail" aria-hidden="true"></a>
    <br>   
    <button>Shop Now</button>
</div>
</center>
<footer>
    <h4><i class="fa fa-copyright" aria-hidden="true">mia@2025</i></h4>
</footer>

</body>
</html>